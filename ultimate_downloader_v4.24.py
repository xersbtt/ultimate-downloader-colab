...
# --- UI ELEMENTS ---
# Auto-populate from Colab secrets if available
...
input_ui = widgets.VBox([
    widgets.HTML("<h3>🚀 Ultimate Downloader v4.24</h3>"),
...
def normalize_playlist_range(range_str):
    ...
    Removes spaces and validates format. Returns None if empty (download all).
    
    yt-dlp accepts: '1', '1,2,3', '1-5', '1,3-5,7'
    """
    if not range_str or not range_str.strip():
        return None  # None means "all items"
    
    # Remove spaces, yt-dlp handles the rest
...
def is_safe_path(base_dir: str, filename: str) -> bool:
    ...
    try:
        ...
        # Fix: Add os.sep to ensure /content/temp_evil doesn't match /content/temp
...
def process_youtube_link(url, mode="video"):
    ...
    archive_path = f"{DRIVE_BASE}yt_history.txt"
    
    # Normalize playlist range for yt-dlp
    ...
    ydl_opts = {
        ...
        'noprogress': True,
        'download_archive': archive_path,  # Prevents re-downloading across sessions
    }
    
    # Add playlist_items filter if specified (yt-dlp only extracts these items)
    if playlist_items:
        ...
        for i, url in enumerate(urls, 1):
            ...
            if "mega.nz" in url or "transfer.it" in url: process_mega_link(url)
                ...
                for u, n in resolve_gofile(url, s, t): 
                    f = download_with_aria2(u, n, COLAB_ROOT, t.get('token'))
                    if f: handle_file_processing(f)
            elif "pixeldrain.com" in url:
                for u, n in resolve_pixeldrain(url, s): 
                    f = download_with_aria2(u, n, COLAB_ROOT)
                    if f: handle_file_processing(f)
                ...
                f = download_with_aria2(url, os.path.basename(unquote(urlparse(url).path)), COLAB_ROOT)
                if f: handle_file_processing(f)
        print(f"\n✅ All Tasks Finished")
    except Exception as e: print(f"\n❌ Critical Error: {e}")
    finally: 
        btn.disabled = False; btn_subs.disabled = False
        reset_progress() # Fix: Use new reset helper
