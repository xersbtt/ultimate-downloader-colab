...
# UPDATED: Replaced "Playlist Start" with "Playlist Range"
playlist_selection = widgets.Text(
    description='Playlist Range:', 
    placeholder='e.g. 1,3,5-10 (Empty = All)', 
...
input_ui = widgets.VBox([
    widgets.HTML("<h3>🚀 Ultimate Downloader v4.21 (Range Selection)</h3>"),
...
def setup_environment(needs_mega, needs_ytdlp, needs_aria):
    ...
    pkg_map = { "unrar": "unrar", "p7zip-full": "7z", "megatools": "megadl", "aria2": "aria2c", "ffmpeg": "ffmpeg" }
    ...
    if to_install:
        ...
        subprocess.run(["apt-get", "install", "-y"] + to_install, check=True, stdout=subprocess.DEVNULL)
    else:
...
def process_youtube_link(url, mode="video"):
    ...
    ydl_opts = {
        ...
        # UPDATED: Removed 'playliststart', added range selection logic below
    }
    
    # NEW: Handle custom playlist ranges (e.g., "1,5,10-20")
    selection = playlist_selection.value.strip()
    if selection:
        ydl_opts['playlist_items'] = selection
        print(f"      🎯 Custom Playlist Range: {selection}")
    ...
    try:
        with yt_dlp.YoutubeDL(ydl_opts) as ydl:
            ...
            # Note: yt-dlp might filter entries internally based on playlist_items, 
            # so len(entries) might reflect the filtered count automatically.
            total_items = len(entries)
            print(f"   📜 Playlist found: {total_items} items")
...
def process_mega_link(url):
    ...
    try:
        ...
        if process.returncode == 0:
            ...
            print(f"   ❌ Mega Error (Code {process.returncode})")
    except Exception as e: 
...
def download_with_aria2(url, filename, dest_folder, cookie=None):
    filename = sanitize_filename(filename)
    if check_duplicate_in_drive(filename): return None
    ...
    for attempt in range(1, 4):
        try:
            ...
            if process.returncode == 0 and os.path.exists(final_path): 
                progress_bar.bar_style = 'info'; return final_path
            else: 
                print(f"      ⚠️ Retry {attempt}/3 - Download incomplete"); time.sleep(2**attempt)
        except Exception as e:
            print(f"      ❌ Download error: {str(e)[:80]}"); break
    return None
...
def handle_file_processing(file_path, source="generic"):
    ...
    for f_path in archive_files:
        ...
        if not is_safe_path(extract_temp, f_path): continue
        extracted_count += 1
        ...
        if os.path.exists(extracted_full) and not os.path.isdir(extracted_full):
            ...
            if os.path.exists(final_dest): os.remove(extracted_full); continue
            if not os.path.exists(os.path.dirname(final_dest)): os.makedirs(os.path.dirname(final_dest))
...
def resolve_gofile(url, s, t):
    try:
        ...
        if not match: return []
        r = s.get(f"https://api.gofile.io/contents/{match.group(1)}", params={'wt': t['wt']}, headers={'Authorization': f"Bearer {t['token']}"}, timeout=30)
        data = r.json()
...
def process_rd_link(link, key):
    ...
    if "magnet:?" in link:
        ...
        try:
            ...
            if 'error' in r: print(f"   ❌ RD Magnet Error: {r.get('error')}"); return
    ...
    try:
        ...
        if 'error' in d: print(f"   ❌ RD Error: {d.get('error')}"); return
...
def execute_batch(mode):
    ...
    try:
        ...
        needs_ytdlp = any(h in u for u in urls for h in ['youtube.com', 'youtu.be', 'twitch.tv', 'vimeo.com'])
    ...
    finally: btn.disabled = False; btn_subs.disabled = False; reset_progress()
