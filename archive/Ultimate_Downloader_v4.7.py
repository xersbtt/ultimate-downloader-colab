...
text_area = widgets.Textarea(description='Links:', placeholder='Paste Links Here (Mega, YouTube, Twitch, etc.)...', layout=widgets.Layout(width='98%', height='150px'))
...
input_ui = widgets.VBox([
    widgets.HTML("<h3>🚀 Ultimate Downloader v4.7 (Final Golden Copy)</h3>"),
...
def determine_destination_path(filename, source="generic"):
    ...
    # DETECT MULTI-PART SUFFIX (Part 1/2, 上篇/下篇)
    part_suffix = ""
...
def setup_environment():
    ...
    if not shutil.which('aria2c') or not shutil.which('megadl'):
        print("🛠️ Installing tools (aria2, megatools)...")
...
def process_youtube_link(url, mode="video"):
    ...
    ydl_opts = {
        ...
        'restrictfilenames': False, # Allow Chinese/Unicode
        'ignoreerrors': True,
    ...
    try:
        with yt_dlp.YoutubeDL(ydl_opts) as ydl:
            try: info = ydl.extract_info(url, download=False)
                ...
                print(f"      ❌ Skipped (Not supported): {url}")
                return
    ...
    try:
        ...
        if process.returncode == 0:
            ...
            for f in os.listdir('/content/'):
                if f not in ['sample_data', '.config', 'drive', 'temp_extract']:
    ...
    for attempt in range(1, 4):
        try:
            ...
            if res.returncode == 0 and os.path.exists(final_path):
                ...
                if attempt < 3: 
                    if "404" not in res.stderr: time.sleep(2**attempt)
                else: print(f"      ❌ Aria2 Error: {res.stderr.strip()[:100]}")
        except subprocess.TimeoutExpired:
            print(f"      ❌ Download Timed Out (Hang Detected).")
            break
        except: break
