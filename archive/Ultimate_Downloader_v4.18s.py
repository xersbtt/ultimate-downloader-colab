...
input_ui = widgets.VBox([
    widgets.HTML("<h3>🚀 Ultimate Downloader v4.17 (Progress+ Edition)</h3>"),
...
def clean_show_name(name):
    ...
    # Remove "End" markers
    name = re.sub(r'(?i)\s+\b(END|FINALE|FINAL)\b
...
def determine_destination_path(filename, source="generic", dry_run=False):
    ...
    is_tv = False
    show_name = "Unknown Show"  # Initialize to prevent undefined variable
...
def setup_environment(needs_mega, needs_ytdlp, needs_aria):
    ...
    for pkg in apt_packages:
        check_cmd = pkg if pkg not in ["p7zip-full"] else "7z"
    ...
    try:
        with yt_dlp.YoutubeDL(ydl_opts) as ydl:
            ...
            for i, entry in enumerate(entries, 1):
                ...
                # Get actual extension that will be used
...
def safe_extract_path(base, filename):
...
def handle_file_processing(file_path, source="generic"):
    ...
    for f_path in archive_files:
        ...
        try:
            safe_path = safe_extract_path(extract_temp, f_path)
        ...
        extracted_full = safe_path  # Use validated path
