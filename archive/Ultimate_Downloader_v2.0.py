...
def determine_destination_path(filename):
    ...
    Supports standard S01E01 and Asian Drama 'Ep01' formats.
    """
    ...
    # 2. LOOSE TV CHECK (Exx / Ep01) - Common in Asian Dramas
    ...
    if sxe_strict:
        ...
        # Implied Season 1 (Asian Drama style)
        season_num = 1 
        
        show_name = filename[:sxe_loose.start()]
        show_name = re.sub(r'[._-]', ' ', show_name).strip()
    else:
        # No TV pattern -> Assume MOVIE
        sxe_strict = None
        sxe_loose = None
...
def handle_file_processing(file_path):
    ...
    # Proper extension handling (Fixes .7z cosmetic bug)
    ...
    for f_path in files:
        ...
        if os.path.exists(extracted_full) and not os.path.isdir(extracted_full):
            ...
            # SMART JUNK FILTER (Deletes small files EXCEPT subtitles)
            if f_size_mb < MIN_FILE_SIZE_MB and file_ext not in KEEP_EXTENSIONS:
                print(f"      -> 🗑️ Skipped junk ({f_size_mb:.1f} MB): {os.path.basename(f_path)}")
